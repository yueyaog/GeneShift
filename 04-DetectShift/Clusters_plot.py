...
for c, IDs in enumerate(IDs_split):
    ...
    plt.tight_layout()
    plt.savefig('{}_gene_exp_fig_{}.png'.format(args.output_path_prefix,c+1),dpi=300)



# Plot the gene expression clustering results on one plot

# plt.figure(figsize=(50,50))
# subplots = int(math.sqrt(ClstNum))+1
# for num,cluster in enumerate(sorted(cluster_list)):
#     ax = plt.subplot(subplots,subplots,num+1)
#     genes = cluster_df[cluster_df['Cluster']==cluster].index
#     for gene in genes:
#         gene_exp = np.array(GEMinput_df.loc[gene])
#         ax.plot(t,gene_exp.ravel(),'k-',alpha=0.15)
#     ax.plot(t,clf.centroids_[num], "r-")
#     ax.set_xticks(t)
#     ax.set_xticklabels(t_label)
#     ax.set_ylabel('Gene expression')
#     ax.set_title('{} ({})'.format(cluster,len(cluster_df[cluster_df['Cluster']==cluster])))
# plt.tight_layout()
# plt.savefig('{}_Clustering.png'.format(args.output_path_prefix),dpi=200)