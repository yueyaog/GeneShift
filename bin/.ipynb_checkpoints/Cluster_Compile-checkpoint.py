...
parser.add_argument('-o','--outputDIR',dest="outputdir",action='store',required=True,help="Output Directory")
args = parser.parse_args()
#########################################################################################

##########################################################################################
DPGP_output = '{}/DTW-K{}-Cluster{}_optimal_clustering.txt'
...
for i in range(1,args.Kval+1):
    df = pd.read_csv(DPGP_output.format(args.inputdir,args.Kval,i),sep='\t',index_col='cluster')
    print('DTW-K{}_Cluster{} DPGP fine clustering output exists.\nReading DPGP Fine Clustering Results.'.format(args.Kval,i))
    for index,row in df.iterrows():
        ...
        DTWDPGP_Results_df.to_csv('{}/DTW-K{}-DPGP_ResultsSum.csv'.format(args.outputdir,str(args.Kval)))